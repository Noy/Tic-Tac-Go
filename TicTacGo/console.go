...
func (c console) PromptMainMenu() int {
	...
	case 1:
		return PLAYER_FIRST
	case 2:
		return CONSOLE_FIRST
	}
	return EXIT_GAME
}

func (c console) DisplayAvailableSpaces(b *GameBoard) {
	rows := boardToASCII(b)
...
func (c console) DisplayBoard(b *GameBoard) {
	rows := boardToASCII(b)
...
func promptForInput(c console, message string, filter ...int) int {
	for {
		...
		if len(filter) == 0 { return conv } else if integerArrayPosition(filter, conv) > -1 { return conv }
	}
	return 0
}

func boardToASCII(board *GameBoard) []string {
	rows := make([]string, 3)
	for i := range rows {
...
func availableSpacesToASCII(board *GameBoard) []string {
	...
	rows := make([]string, 3)
	for i := range rows { rows[i] = strings.Join(indices[i*3:i*3+3], " ") }
	return rows
}
